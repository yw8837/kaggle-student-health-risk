...
6. **최종 성적**: 정직 OOF 최고 **0.95088** (N9: HistGB-TE 0.15 + RealMLP 0.57 + K 0.28, 중첩검증) / 제출 실측 LB 0.95007 (N5). 34종 실험 전수에서 표현(exact-value TE)만이 유일한 실질 레버(+0.0008), 파생·튜닝·앙상블·노이즈·계층 축은 전부 ±0.0002 이내
...
- 1단계(PoC): 설문 기반 배치 스코어링 + 담당자 대시보드
- 2단계: 학생앱 연동 실시간화, 개입 효과 A/B 추적
- 운영 지표: 소수클래스 recall (놓친 위험학생 최소화), 개입 후 등급 개선율
- 윤리·프라이버시: 건강정보 동의 기반 수집, 라벨링 낙인효과 방지(본인+상담사만 열람)
