...
def main():
    print("=== A_3.py: 0.953+ 초극강 앙상블 (LightGBM + HistGB) ===")
    t0 = time.time()
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
        ...
        test_lgb += model_lgb.predict_proba(X_te) / n_splits
        ...
        test_hgb += model_hgb.predict_proba(X_te) / n_splits
    ...
    oof_blend = oof_lgb * 0.5 + oof_hgb * 0.5
    test_blend = test_lgb * 0.5 + test_hgb * 0.5
    ...
    final_test_preds = apply_rule(test_raw, test_blend)
    
    classes = ['at-risk', 'unhealthy', 'fit']
    sub = pd.DataFrame({'id': test_raw['id'], 'health_condition': [classes[p] for p in final_test_preds]})
    sub.to_csv('A_3.csv', index=False)
    print(f"\n최종 제출 파일 생성 완료: A_3.csv (총 소요시간: {time.time()-t0:.1f}초)")
