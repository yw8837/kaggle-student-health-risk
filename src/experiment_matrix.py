...
ROWS = [
    ...
    ("N3", "3모델 보팅", "서수+수치TE", "+규칙피처", "가중", "전체×5fold", "중첩검증", None, None, None),
]
...
def main():
    ...
    tbl.scale(1, 1.45)
    best = df["BalancedAcc"].max()
    for (r, c), cell in tbl.get_celld().items():
        if r == 0:
            cell.set_facecolor("#1f2937"); cell.set_text_props(color="white", fontweight="bold")
        elif df.iloc[r-1]["BalancedAcc"] == best:
            cell.set_facecolor("#dcfce7")           # 최고점 행 강조
        elif df.iloc[r-1]["실험"] in ("N1", "N2", "N3"):
